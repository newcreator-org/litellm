...
const ConditionalPublicModelName: React.FC = () => {
  ...
  const publicNameTooltipContent = (
    <>
      <div className="mb-2 font-normal">The name you specify in your API calls to Connect LM Proxy</div>
      <div className="mb-2 font-normal">
        ...
        <code className="bg-gray-700 px-1 py-0.5 rounded text-xs">openai/qwen-plus-latest</code> as the Connect LM model
      </div>
      <div className="mb-2 font-normal">
        <strong>Usage:</strong> You make an API call to the Connect LM proxy with{" "}
        <code className="bg-gray-700 px-1 py-0.5 rounded text-xs">model = &quot;example-name&quot;</code>
      </div>
      <div className="font-normal">
        <strong>Result:</strong> Connect LM sends{" "}
        <code className="bg-gray-700 px-1 py-0.5 rounded text-xs">qwen-plus-latest</code> to the provider
      </div>
    </>
  );

  const liteLLMModelTooltipContent = <div>The model name Connect LM will send to the LLM API</div>;
  ...
  return (
    <>
      <Form.Item
        ...
        name="model_mappings"
        tooltip="Map public model names to Connect LM model names for load balancing"
        labelCol={{ span: 10 }}
        wrapperCol={{ span: 16 }}
        labelAlign="left"
        rules={[
