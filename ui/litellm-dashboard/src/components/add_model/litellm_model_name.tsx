...
const LiteLLMModelNameField: React.FC<LiteLLMModelNameFieldProps> = ({
  ...
  return (
    <>
      <Form.Item
        label="Connect LM Model Name(s)"
        tooltip="The model name Connect LM will send to the LLM API"
        className="mb-0"
      >
        <Form.Item
          name="model"
      ...
      <Row>
        ...
        <Col span={14}>
          <Text className="mb-3 mt-1">
            {selectedProvider === Providers.Azure
              ? "Your deployment name will be saved as the public model name, and Connect LM will use 'azure/deployment-name' internally"
              : "The model name Connect LM will send to the LLM API"}
          </Text>
        </Col>
      </Row>
    </>
